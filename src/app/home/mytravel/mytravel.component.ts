...
export class MytravelComponent implements OnInit {
  ...
  onClick = (object: any, typeButton: string) => {
    ...
    if (typeButton === 'rejectOne') {
      this.tr.postTravel(travelId, 1, userOperation, cadeteId, true).subscribe(
        (resp) => {
          Swal.fire({
      ...
      this.tr
        .postTravel(travelId, 3, userOperation, cadeteId, isReasigned)
        .subscribe(
          (resp) => {
            Swal.fire({
      ...
      this.tr
        .postTravel(travelId, 4, userOperation, cadeteId, isReasigned)
        .subscribe((resp) => {
          Swal.fire({
      ...
      this.tr
        .postTravel(travelId, 7, userOperation, cadeteId, isReasigned)
        .subscribe(
          (resp) => {
            Swal.fire({
              title: 'Has retirado el equipo correctamente',
      ...
      this.tr
        .postTravel(travelId, 8, userOperation, cadeteId, isReasigned)
        .subscribe(
          (resp) => {
            Swal.fire({
              title: 'Has entregado el equipo a Todoit correctamente',
      ...
      this.tr.postTravel(travelId, 5, userOperation, cadeteId, true).subscribe(
        (resp) => {
          Swal.fire({
            title: 'Rechazado correctamente',
            icon: 'success',
  ...
  loadTravels() {
    ...
    forkJoin([two, three, six, seven]).subscribe((results) => {
      this.travels = [
        ...results[0],
        ...results[1],
        ...results[2],
